...
def search_detail_html(name, kind_label, lat, lon, map_hash, enrich=None):
    ...
    _http_ok = _re_url.compile(r"^https?://", _re_url.I)
...
def create_zim(
    ...
    with creator:
        ...
        # `name` usually already reads "OSM - <Region>"; strip that so the
        # Name isn't "osm_osm_-_region".
        zim_name = _re_name.sub(r"^osm\s*-\s*", "", name.strip(), flags=_re_name.I)
