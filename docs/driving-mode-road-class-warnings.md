...
| 9      | no motor vehicles: footway/path/steps/pedestrian/cycleway/bridleway/corridor/escape/busway, or access/vehicle/motor_vehicle ∈ {no, private} without an explicit motor_vehicle allow. The car router never expands these edges (routing-worker.js, index.html, tests/szrg_astar.py, cloud/route_cli.py). Builders before 2026-09 leave it clear, so consumers also treat class ordinals 16..20 (path..steps) as no-motor. |
